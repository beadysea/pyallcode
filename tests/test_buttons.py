...
def test_pushbuttons_read():
    from pyallcode.devices.buttons import PushButtons

    conn = FakeConnection()
    conn.to_return = [1, 0]
    btn = PushButtons(conn)

    assert btn.read(0) is True
    assert btn.read(1) is False

    assert conn.calls[0][0] == 'ReadSwitch 0'
    assert conn.calls[1][0] == 'ReadSwitch 1'
